...
import { BellIcon, ChatIcon, CursorIcon, DownArrowIcon, PeopleIcon, ReactionIcon } from "../svgs";
import usePaperStore from "@/store/paper_store";

type Props = {};

export default function TopRightMenu({}: Props) {
  const { setRightSideBarType, rightSideBarType, showCursorPartner, setShowCursorPartner } =
    usePaperStore();
  return (
    <div className=' bg-white rounded-md shadow-lg pointer-events-auto'>
      <div className='h-11 flex items-center'>
        ...
        <div
          className={`ct-menu-item ${rightSideBarType === "comment" && "active"}`}
          onClick={() => {
        ...
        <div
          className={`ct-menu-item ${rightSideBarType === "message" && "active"}`}
          onClick={() => {
