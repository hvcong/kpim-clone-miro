...
export default function LeftSideBar({}: Props) {
  ...
  return (
    <div
      className={`absolute left-2 pointer-events-none bottom-2 top-14 mt-1 flex  transition-all duration-300   ${
    ...
    >
      <div className='w-[340px] bg-white rounded-md shadow-lg pointer-events-auto flex flex-col'>
        <div className='flex-1 flex flex-col overflow-y-hidden'>
          <div className='flex-1 overflow-y-hidden flex flex-col'>{renderBody()}</div>
        </div>
        <div className='flex relative mx-3'>
          <div
            className={`absolute top-0 h-0.5 bg-blue-600 transition-all duration-500 ${
              leftSideBarType === "frame" ? "left-0 right-1/2" : "left-1/2 right-0"
            }`}
          ></div>
          <div
          ...
          >
            <HistoryIcon
              className={`w-6 h-6  group-hover:text-blue-600 ${
                leftSideBarType === "history" ? "text-blue-600" : "text-gray-800"
              }`}
            />
          </div>
        </div>
